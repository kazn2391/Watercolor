...
export async function rewritePdfDownloadLink(
  ...
  for (let i = 0; i < annotsRaw.size(); i++) {
    ...
    // Sadece buyuk DOWNLOAD-iliskili linkleri degistir.
    // GIFT (kucuk, area ~1277) ve Etsy bundle linkleri (kucuk) korunur.
    if (area <= 20000) continue;

    // Yeni temiz URI action yaz (eskiyi okumaya calisma)
  ...
  if (changed === 0) {
    throw new Error('DOWNLOAD butonu bulunamadi (buyuk link yok)');
  }
