...
export default function DoctorPage() {
  ...
  const [pngMode, setPngMode] = useState(false);
  const [diagnosing, setDiagnosing] = useState(false);
  const [applying, setApplying] = useState(false);
  const [result, setResult] = useState<any>(null);
  const [pngResult, setPngResult] = useState<any>(null);
  ...
  const handleDiagnose = async () => {
    ...
    try {
      if (pngMode) {
        if (!driveUrl) {
          setError('PNG modu icin Drive klasor linki gerekli');
          setDiagnosing(false);
          return;
        }
        const pngRes = await fetch('/api/doctor/png?key=' + ADMIN_KEY, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ driveUrl }),
        });
        const pngData = await pngRes.json();
        setPngResult(pngData);
        if (!pngRes.ok) {
          setError('PNG uretimi basarisiz: ' + (pngData.error || 'hata'));
          setDiagnosing(false);
          return;
        }
      }
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) setError(data.error || 'Update hatasi');
    ...
    <div style={{ maxWidth: 1100, margin: '20px auto', padding: 20, fontFamily: 'sans-serif' }}>
      ...
      <div style={{ background: '#f5f5f5', padding: 20, borderRadius: 8, marginBottom: 20 }}>
        ...
        <div style={{ marginBottom: 12, paddingTop: 8, borderTop: '1px solid #ddd' }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontWeight: 'bold', cursor: 'pointer' }}>
            <input
              type="checkbox"
              checked={pngMode}
              onChange={(e) => setPngMode(e.target.checked)}
            />
            🎨 PNG Uret + SEO'yu PNG Moduna Cevir
          </label>
        </div>

        {pngMode && (
          <div style={{ marginBottom: 12 }}>
            <label style={{ display: 'block', marginBottom: 4, fontWeight: 'bold' }}>Drive Klasor URL:</label>
            <input
              type="text"
              value={driveUrl}
            ...
            <p style={{ fontSize: 12, color: '#888', marginTop: 4 }}>
              Bu klasordeki gorseller PNG'ye cevrilip "Png" alt klasorune yuklenecek.
            </p>
          </div>
        )}
        ...
        >
          {diagnosing ? (pngMode ? 'PNG Uretiliyor + Tani...' : 'Tani Konuyor...') : '🩺 Tani Koy'}
        </button>
      </div>
      ...
      {pngResult && (
        <div style={{
          background: pngResult.success ? '#e7ffe7' : '#fff3cd',
          padding: 12,
          borderRadius: 4,
          marginBottom: 16,
          fontSize: 13,
          ...
          {pngResult.success && (
            <strong>PNG uretildi: {pngResult.pngCount} basarili
              {pngResult.failed > 0 ? ', ' + pngResult.failed + ' hatali' : ''}</strong>
          )}
          {(pngResult.steps || []).map((s: string, i: number) => (
            <div key={i} style={{ color: '#555' }}>{s}</div>
          ))}
        </div>
      )}
      ...
      {result && (
        <div>
          <div style={{
            ...
            {result.pngMode && (
              <p style={{ color: '#080', fontWeight: 'bold' }}>
                🎨 PNG modu: SEO transparent PNG icin optimize edildi
              </p>
            )}
            {result.diagnosis.isBestsellerWarning && (
              <p style={{ color: '#856404', fontWeight: 'bold' }}>
                ⚠️ Bu listing iyi performans gosteriyor. Degisiklikleri dikkatlice incele!
              </p>
            )}
